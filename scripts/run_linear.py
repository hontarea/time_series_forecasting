"""
SL producer — LTSF-Linear (spec §5.6).
...
Usage:
    python scripts/run_linear.py data/final_dataset/BTCUSDT.csv
    python scripts/run_linear.py data/final_dataset/ETHUSDT.csv --store-root forecasts
    python scripts/run_linear.py data/final_dataset/BTCUSDT.csv --device cuda  # fail loud if no GPU
"""
...
MODEL = "linear"   # fixed per script (spec §5.6)
SEED = 0           # fixed per script (single-seed regime for now)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Produce LTSF-Linear forecasts for one asset into the "
