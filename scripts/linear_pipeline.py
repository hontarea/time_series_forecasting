...
Usage:
    python scripts/linear_pipeline.py
...
def _build_model(dataset: Dataset, lr: float = LR, epochs: int = EPOCHS) -> TorchAdapter:
    """Construct a LinearModel wrapped in TorchAdapter."""
...
def run_baseline(
    ...
    """Run walk-forward with default Linear model and backtest."""
...
def plot_results(
    ...
    axes[0].plot(strategy_cum.values, label="Strategy (Linear)", color="steelblue")
    axes[0].plot(
        market_cum.values, label="Market (Buy & Hold)",
        color="gray", linestyle="--", alpha=0.8,
    )
    axes[0].axhline(0, color="black", linewidth=0.5)
    axes[0].set_title("Linear Model — Cumulative Return: Strategy vs Market")
    ...
    plt.tight_layout()
    if save_dir:
        fig.savefig(save_dir / "linear_baseline.png", dpi=150)
        print(f"   Saved: {save_dir / 'linear_baseline.png'}")
    plt.show()
    ...
    if optimised_result:
        ...
        ax.set_title("Linear — Baseline vs Optimised vs Market")
        ...
        plt.tight_layout()
        if save_dir:
            fig.savefig(save_dir / "linear_comparison.png", dpi=150)
            print(f"   Saved: {save_dir / 'linear_comparison.png'}")
        plt.show()


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Linear model pipeline for BTC/USDT 1H forecasting.",
    )
    parser.add_argument(
