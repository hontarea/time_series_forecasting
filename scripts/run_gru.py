...
Usage:
    python scripts/run_gru.py data/final_dataset/BTCUSDT.csv
    python scripts/run_gru.py data/final_dataset/ETHUSDT.csv --store-root forecasts
    python scripts/run_gru.py data/final_dataset/BTCUSDT.csv --device cuda  # fail loud if no GPU
"""
...
MODEL = "gru"      # fixed per script (spec §5.6)
SEED = 0           # fixed per script (single-seed regime for now)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Produce GRU forecasts for one asset into the "
