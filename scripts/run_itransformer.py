"""
SL producer — iTransformer (spec §5.6).
...
Usage:
    python scripts/run_itransformer.py data/final_dataset/BTCUSDT.csv
    python scripts/run_itransformer.py data/final_dataset/ETHUSDT.csv --store-root forecasts
    python scripts/run_itransformer.py data/final_dataset/BTCUSDT.csv --device cuda  # fail loud if no GPU
"""
...
MODEL = "itransformer"   # fixed per script (spec §5.6)
SEED = 0                 # fixed per script (single-seed regime for now)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Produce iTransformer forecasts for one asset into the "
